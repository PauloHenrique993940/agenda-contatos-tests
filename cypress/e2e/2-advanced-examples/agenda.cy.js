...
describe('Testando Funcionalidades', () => {
  ...
  it('Alterando Contatos', () => {
    ...
    cy.get('[type="email"]').clear().type("joaobarbosa@teste.com")
    cy.get('[type="tel"]').clear().type("62 9 99999999")
    cy.get('.ckeKmo button.alterar').click()
  
    cy.wait(500);
    cy.get(':nth-child(3) > .sc-gueYoa > .edit').click()
    cy.get('[type="text"]').clear().type("Maria Fernanda")
    cy.get('[type="email"]').clear().type("mariafernanda@teste.com")
    cy.get('[type="tel"]').clear().type("62 9 88888888")
    cy.get('.ckeKmo button.alterar').click()
  
    cy.wait(500);
    cy.get(':nth-child(4) > .sc-gueYoa > .edit').click()
    cy.get('[type="text"]').clear().type("Pedro Augusto")
    cy.get('[type="email"]').clear().type("pedroaugusto@teste.com")
    cy.get('[type="tel"]').clear().type("62 9 77777777")
    cy.get('.ckeKmo button.alterar').click()
  })
